// Copyright 2022 OpenHW Group
// Solderpad Hardware License, Version 2.1, see LICENSE.md for details.
// SPDX-License-Identifier: Apache-2.0 WITH SHL-2.1

// Primitives
module ihp_sg13g2_clk_gating (
  input  logic clk_i,
  input  logic en_i,
  input  logic test_en_i,
  output logic clk_o
);

  sg13g2_slgcp_1 clk_gate_inst (
    .CLK(clk_i),
    .GATE(en_i),
    .SCE(test_en_i),
    .GCLK(clk_o)
  );

endmodule

module ihp_sg13g2_clk_inverter (
  input  logic clk_i,
  output logic clk_o
);

  sg13g2_inv_1 clk_inv_inst (
    .A(clk_i),
    .Y(ckl_o)
  );

endmodule

module ihp_sg13g2_clk_mux2 (
  input  logic clk0_i,
  input  logic clk1_i,
  input  logic clk_sel_i,
  output logic clk_o
);

  sg13g2_mux2_1 clk_mux2_inst (
    .A0(clk0_i),
    .A1(clk1_i),
    .S(clk_sel_i),
    .X(clk_o)
  );

endmodule

module ihp_sg13g2_clk_xor2 (
  input  logic clk0_i,
  input  logic clk1_i,
  output logic clk_o
);

  sg13g2_xor2_1 clk_xor2_inst (
    .A(clk0_i),
    .B(clk1_i),
    .X(clk_o)
  );

endmodule

// Cluster
module cluster_clock_inverter(
  input  logic clk_i,
  output logic clk_o
);

  ihp_sg13g2_clk_inverter clk_inv_inst (
    .*
  );

endmodule

// Pulp
module pulp_clock_mux2 (
  input  logic clk0_i,
  input  logic clk1_i,
  input  logic clk_sel_i,
  output logic clk_o
);

  ihp_sg13g2_clk_mux2 clk_mux2_inst (
    .*
  );

endmodule

module pulp_clock_inverter (
  input  logic clk_i,
  output logic clk_o
);

  ihp_sg13g2_clk_inverter clk_inv_inst (
    .*
  );

endmodule

// OpenHW
module cv32e40p_clock_gate (
  input  logic clk_i,
  input  logic en_i,
  input  logic scan_cg_en_i,
  output logic clk_o
);

  ihp_sg13g2_clk_gating clk_gate_inst (
    .clk_i,
    .en_i,
    .test_en_i(scan_cg_en_i),
    .clk_o
  );

endmodule

module cve2_clock_gate (
  input  logic clk_i,
  input  logic en_i,
  input  logic scan_cg_en_i,
  output logic clk_o
);

  ihp_sg13g2_clk_gating clk_gate_inst (
    .clk_i,
    .en_i,
    .test_en_i(scan_cg_en_i),
    .clk_o
  );

endmodule

// TC
module tc_clk_gating #(
    parameter bit IS_FUNCTIONAL = 1'b1
  ) (
    input  logic clk_i,
    input  logic en_i,
    input  logic test_en_i,
    output logic clk_o
);

  ihp_sg13g2_clk_gating clk_mux2_inst (
    .*
  );

endmodule

module tc_clk_xor2 (
  input  logic clk0_i,
  input  logic clk1_i,
  output logic clk_o
);

  ihp_sg13g2_clk_xor2 clk_xor2_inst (
    .*
  );

endmodule

module tc_clk_mux2 (
  input  logic clk0_i,
  input  logic clk1_i,
  input  logic clk_sel_i,
  output logic clk_o
);

  ihp_sg13g2_clk_mux2 clk_mux2_inst (
    .*
  );

endmodule

module tc_clk_inverter (
  input  logic clk_i,
  output logic clk_o
);

  ihp_sg13g2_clk_inverter clk_inv_inst (
    .*
  );

endmodule
