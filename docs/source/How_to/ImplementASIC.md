...
### Run commands
First you need to generate all the template files with the corresponding config and pad files:
```
make mcu-gen TARGET=asic_ihp                                \
             X_HEEP_CFG=configs/python_unsupported.hjson    \
             PYTHON_X_HEEP_CFG=configs/general_librelane.py \
             PADS_CFG=configs/pad_cfg_librelane.py
```
Note that you can change by hand the generated files inside `hw/asic/ihp-sg13g2` to fit you own design and specify SRAMs placement, SDC constraints, PDN,...
...
- The frequency is really low and does not passes STA: this might be due to the incorrect setting of SDC constraints.
