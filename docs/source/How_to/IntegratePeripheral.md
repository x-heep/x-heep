...
└── hw/ip/<peripheral>
    ├── data/
    │   └──<peripheral>.hjson           # register description (input to regtool)
    ├── rtl/
    │   └──<peripheral>.sv              # RTL of the peripheral
    ├── <peripheral>.core               # FuseSoC description (includes regtool generate block)
    └── <peripheral>.vlt                # Verilator waiver
```

The `<peripheral>_reg_top.sv` and `<peripheral>_reg_pkg.sv` RTL files are generated on the fly by the FuseSoC [`regtool` generator](RegGenerator.md) — they do not need to be committed to the repository.
...
This tool generates both RTL and SW components from an HJSON register description. X-HEEP integrates it through the FuseSoC [`regtool` generator](RegGenerator.md) (`xheep:util:reg-generator`), which runs `regtool` automatically during every `fusesoc` invocation — no separate script call or checked-in generated files required.
...
2. Declare a `generate` block in `<peripheral>.core` that invokes the `regtool` generator. See [FuseSoC reg-generator](RegGenerator.md) for the full parameter reference and a ready-to-use `.core` template.

## 3. Expose the IP to FuseSoC
This is a crucial step in integrating the new peripheral in X-HEEP's flow:

1. Adapt `hw/ip/<peripheral>/<peripheral>.core` from [`dlc.core`](../../../hw/ip_examples/dlc/dlc.core). `dlc.core` already uses the [`regtool` FuseSoC generator](RegGenerator.md) — list your hand-written RTL files under `files:`, adjust the `generate` block parameters, and declare the required dependencies:
   ```yaml
   filesets:
     files_rtl:
       depend:
   ...
   generate:
     <peripheral>-regs:
       ...
       parameters:
         name:     <peripheral>
         config:   data/<peripheral>.hjson
         ...
   ```
   Any generated `_reg_pkg.sv` / `_reg_top.sv` files are added as dependencies automatically — do not list them under `files:`. See [FuseSoC reg-generator](RegGenerator.md) for all available parameters (C structs header, Mako templates, caching, etc.).
