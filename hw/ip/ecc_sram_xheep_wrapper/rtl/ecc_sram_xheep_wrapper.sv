...
//
// File: ecc_sram_xheep_wrapper.sv
// Author: Flavia Guella
// Date: 13/07/2026
// Description: Modified ecc_sram.sv from PULP Platform redundancy cells. Instantiate sram_wrapper.sv instead of tc_sram.sv


// ECC-protected SRAM, with enconding and decoding (if needed), bytewise access, and scrubbing

module ecc_sram_xheep_wrapper #(
  ...
  assign valid_read_d   = req_i && gnt_o && (~we_i || (be_i != {ByteEnWidth{1'b1}}));
...
`endif
  ...
  sram_wrapper #(
      ...
      .clk_i(clk_i),
      .rst_ni(rst_ni),
      .req_i(bank_scrub_req),
      .we_i(bank_scrub_we),
      .addr_i(bank_scrub_addr),
      .wdata_i(bank_scrub_wdata),
      .be_i('1),  // be always set (always write the whole word)
      .pwrgate_ni(pwrgate_ni),
      .pwrgate_ack_no(pwrgate_ack_no),
      .set_retentive_ni(set_retentive_ni),
      .rdata_o(bank_scrub_rdata)
  );
