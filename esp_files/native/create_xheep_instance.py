...
def third_party_flow(instance_num):
    """
    ...
    source_dir = esp_root / "accelerators" / "rtl" / "xheep_rtl" / "vendor" / "x-heep"
    new_instance_name = f"xheep{instance_num}"
    new_instance_dir = esp_root / "accelerators" / "third-party" / new_instance_name
    ...
    print(f"\nCopying {source_dir} -> {new_instance_dir}...")
    shutil.copytree(source_dir, new_instance_dir, 
                    ignore=shutil.ignore_patterns('create_xheep_instance.py', 'out', 'esp_files'))
...
def native_flow():
    ...
    with open(makefile_path, 'r') as f:
        lines = f.readlines()
    lines[0] = f"ACCEL_NAME := {new_name}_rtl\n"
    with open(makefile_path, 'w') as f:
        f.writelines(lines)
        
    # Replace occurrences of "xheep" and "XHEEP"
    for subdir in ["hw", "sw"]:
        for dirpath, _, filenames in os.walk(new_accel_dir / subdir):
...
def main():
    flow = input("Which flow are you using? (3rd party / native): ").lower()

    if "3rd" in flow or "party" in flow:
        try:
            instance_num = int(input("Enter the instance number: "))
        except ValueError:
            print("Error: Instance number must be an integer.")
            sys.exit(1)
        success = third_party_flow(instance_num)
        sys.exit(0 if success else 1)
    elif "native" in flow:
        success = native_flow()
        sys.exit(0 if success else 1)
    else:
        print("Error: Invalid flow specified. Please choose '3rd party' or 'native'.")
        sys.exit(1)
