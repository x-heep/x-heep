...
int main (int argc, char * argv[])
{
  ...
  execute_from_ext = cmd_lines_options->get_execute_from_ext();

  if(firmware.empty() && use_openocd==false && !execute_from_ext){
      std::cout<<"You must specify the firmware if you are not using OpenOCD or execute_from_ext"<<std::endl;
      exit(EXIT_FAILURE);
  }
  ...
  dut->execute_from_ext_i   = execute_from_ext ? 1 : 0;
  ...
  if(execute_from_ext) {
    // Application is baked into ext_rom.sv; nothing to load, the CPU jumps
    // there directly out of boot_rom.S.
    std::cout<<"X-HEEP is executing from external device (ext_rom)..."<< std::endl;
  } else {
