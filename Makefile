...
BUILD_LATTICE_DIR         = $(FUSESOC_BUILD_DIR)/icesugar-trellis
VERILATOR_LATTICE_DIR     = $(FUSESOC_BUILD_DIR)/sim-lattice-verilator
...
verilator-build-lattice: | .check-verilator
	$(FUSESOC) --cores-root . run --no-export --target=sim-lattice --tool=verilator $(FUSESOC_FLAGS) --build openhwgroup.org:systems:core-v-mini-mcu $(FUSESOC_PARAM) 2>&1 | tee buildsim.log
...
verilator-run-lattice:
	$(FUSESOC) --cores-root . run --no-export --target=sim-lattice --tool=verilator $(FUSESOC_FLAGS) --run openhwgroup.org:systems:core-v-mini-mcu $(FUSESOC_PARAM) \
		--run_options="+firmware=../../../sw/build/main.hex $(SIM_ARGS)"
...
verilator-waves-lattice: .check-gtkwave
	gtkwave $(VERILATOR_LATTICE_DIR)/waveform.fst
...
## Programs the iCESugar-pro board with the bitstream from lattice-fpga
lattice-prog:
	icesprog $(BUILD_LATTICE_DIR)/*.bit
