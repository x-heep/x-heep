...
#include "xheep_fw_esp_p2p.h"
#include "xheep1_fw_esp_p2p.h"
...
int main(int argc, char **argv)
{
    ...
    xheep_fw_section_t prod_secs[XHEEP1_FW_ESP_P2P_NUM_SECTIONS];
    xheep_fw_section_t cons_secs[XHEEP_FW_ESP_P2P_NUM_SECTIONS];
    for (unsigned s = 0; s < XHEEP1_FW_ESP_P2P_NUM_SECTIONS; ++s) {
        prod_secs[s].addr = xheep1_fw_esp_p2p_sections[s].addr;
        prod_secs[s].size = xheep1_fw_esp_p2p_sections[s].size;
        prod_secs[s].data = xheep1_fw_esp_p2p_sections[s].data;
    }
    for (unsigned s = 0; s < XHEEP_FW_ESP_P2P_NUM_SECTIONS; ++s) {
        cons_secs[s].addr = xheep_fw_esp_p2p_sections[s].addr;
        cons_secs[s].size = xheep_fw_esp_p2p_sections[s].size;
        cons_secs[s].data = xheep_fw_esp_p2p_sections[s].data;
    }
    size_t fw_buffer_size_prod = xheep_fw_size(prod_secs, XHEEP1_FW_ESP_P2P_NUM_SECTIONS);
    size_t fw_buffer_size_cons = xheep_fw_size(cons_secs, XHEEP_FW_ESP_P2P_NUM_SECTIONS);
