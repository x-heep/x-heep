...
def test_github_qualify_references():
    ...
    assert (
        github_qualify_references(items_unqualified, repo_userorg, repo_name)
        == exp_items_unqualified
    )
    ...
    assert (
        github_qualify_references(items_qualified, repo_userorg, repo_name)
        == items_qualified
    )
    ...
    assert (
        github_qualify_references(items_invalid, repo_userorg, repo_name)
        == items_invalid
    )
...
def test_github_parse_url():
    ...
    assert github_parse_url("https://github.com/lowRISC/ibex") == ("lowRISC", "ibex")
    assert github_parse_url("git@github.com:lowRISC/ibex.git") == ("lowRISC", "ibex")
...
def github_parse_url(github_repo_url):
    ...
    regex = (
        r"(?:@github\.com\:|\/github\.com\/)([a-zA-Z\d-]+)\/([a-zA-Z\d-]+)(?:\.git)?$"
    )
...
def produce_shortlog(clone_dir, mapping, old_rev, new_rev):
    ...
    subdirs = ["."] if mapping is None else [m.from_path for m in mapping.items]
...
def format_list_to_str(list, width=70):
    ...
    wrapper = textwrap.TextWrapper(
        initial_indent="* ", subsequent_indent="  ", width=width
    )
...
def get_field(
    path, where, data, name, expected_type=dict, optional=False, constructor=None
):
    value = data.get(name)
    if value is None:
        if not optional:
    ...
    if not isinstance(value, expected_type):
        raise JsonError(
            path,
            "{}, the {!r} field is {!r}, but should be of type {!r}.".format(
                where, name, value, expected_type.__name__
            ),
        )
...
class Mapping1:
    ...
    def apply_patch(basedir, patchfile):
        cmd = ["git", "apply", "--directory", str(basedir), "-p1", str(patchfile)]
        if verbose:
            cmd += ["--verbose"]
        subprocess.run(cmd, check=True)

    def import_from_upstream(
        self, upstream_path, target_path, exclude_files, patch_dir
    ):
...
class Desc:
    ...
    def __init__(self, path, data, desc_overrides):
        where = "at top-level"
        ...
        self.upstream = get_field(
            path, where, data, "upstream", constructor=lambda data: Upstream(path, data)
        )
        self.patch_dir = get_field(
            path,
            where,
            data,
            "patch_dir",
        ...
        for efu in self.exclude_from_upstream:
            if not isinstance(efu, str):
                raise JsonError(
                    path,
                    "exclude_from_upstream has entry {}, which is not a string.".format(
                        efu
                    ),
                )

    @staticmethod
    def parse_vendor_file(handle, desc_overrides, module_filter=None):
        """Parse a vendor file, returning a list of Desc objects.

        Single-entry format (has 'name' at top level): returns one Desc.
        Multi-entry format: each key is a project name; target_dir is
        derived as <vendor>/<project> from the file name.
        """
        ...
        if "name" in data:
            ...
            for project_name, project_data in data.items():
                if not isinstance(project_data, dict):
                    raise JsonError(
                        path,
                        "Project {!r} value must be a dict.".format(project_name),
                    )
                if module_filter and project_name != module_filter:
                    continue
            ...
            if module_filter and not descs:
                raise JsonError(
                    path,
                    "Module {!r} not found in vendor file.".format(module_filter),
                )
            return descs
    ...
    def apply_overrides(self, desc_data, desc_overrides):
        ...
        for key, value in desc_overrides:
            log.info(
                "Overriding description key {!r} with value {!r}".format(key, value)
            )
            ref = desc_data
...
def ignore_patterns(base_dir, *patterns):
    ...
    def _ignore_patterns(path, names):
        ...
        for pattern in patterns:
            pattern_matches = [
                n for n in names if fnmatch.fnmatch(_rel_to_base(path, n), pattern)
            ]
...
def git_add_commit(paths, commit_msg):
    ...
    try:
        subprocess.run(
            cmd_commit, check=True, universal_newlines=True, input=commit_msg
        )
...
def define_arg_type(arg):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(
            "unable to parse {!r}: configuration overrides must be in the form key=value".format(
                arg
            )
        )
    ...
    if key not in OVERRIDABLE_DESC_KEYS:
        raise argparse.ArgumentTypeError(
            "invalid configuration override: key {!r} cannot be overwritten".format(key)
        )
    return (key, value)


def process_vendor(desc, args):
    """Process a single vendor entry: clone, copy, patch, lock, commit."""
    ...
    update = args.update
    if lock is None and not update:
        log.warning("No lock entry for %s, so will update upstream repo.", desc.name)
        update = True

    # If we have a lock file and we're not in update mode, override desc's
    # upstream field with the one from the lock file.
    changed_url = False
    if lock is not None:
    ...
    with tempfile.TemporaryDirectory() as clone_dir:
        ...
        if desc.upstream.only_subdir is not None:
            ...
            if not clone_subdir.is_dir():
                log.fatal(
                    "subdir '{}' does not exist in repo".format(
                        desc.upstream.only_subdir
                    )
                )
                raise SystemExit(1)
        ...
        if update:
            if lock is None:
                get_shortlog = False
                log.warning(
                    "No lock entry for %s: unable to summarize changes.",
                    desc.name,
                )
            elif changed_url:
                get_shortlog = False
                log.warning(
                    "The repository URL changed since the last run. "
                    "Unable to get log of changes."
                )

        shortlog = None
        if get_shortlog:
            shortlog = produce_shortlog(
                clone_subdir, desc.mapping, lock.upstream.rev, upstream_new_rev
            )

            # Ensure fully-qualified issue/PR references for GitHub repos
            gh_repo_info = github_parse_url(desc.upstream.url)
            if gh_repo_info:
                shortlog = github_qualify_references(
                    shortlog, gh_repo_info[0], gh_repo_info[1]
                )

            log.info("Changes since the last import:\n" + format_list_to_str(shortlog))

        # write lock file (append entry if file already exists)
        if update:
            lock_data = {}
            if os.path.exists(str(lock_file_path)):
                with open(str(lock_file_path), "r", encoding="UTF-8") as f:
        ...
        if args.commit:
            ...
            if repo_info is not None:
                sha_short = "%s/%s@%s" % (repo_info[0], repo_info[1], sha_short)

            commit_msg_subject = "Update %s to %s" % (desc.name, sha_short)
            intro = "Update code from {}upstream repository {} to revision {}".format(
                (
                    ""
                    if desc.upstream.only_subdir is None
                    else "subdir {} in ".format(desc.upstream.only_subdir)
                ),
                desc.upstream.url,
                upstream_new_rev,
            )
    ...
    log.info("Import of %s finished", desc.name)
...
def main(argv):
    ...
    parser.add_argument(
        "--commit", "-c", action="store_true", help="Commit the changes"
    )
    parser.add_argument(
        "--desc-override",
        "-D",
    ...
    parser.add_argument(
        ...
        "-m",
        default=None,
        help="Only process the specified module from the vendor file.",
    )
    ...
    # Parse vendor file (supports single-entry and multi-entry formats)
    try:
        descs = Desc.parse_vendor_file(args.desc_file, args.desc_overrides, args.module)
    except (JsonError, ValueError) as err:
        log.fatal(str(err))
        raise SystemExit(1)

    if not descs:
        log.warning("No matching modules found.")
        return

    # Check for a clean working directory when commit is requested
    if args.commit:
        if not git_is_clean_workdir(descs[0].path.parent):
            log.fatal(
                "A clean git working directory is required for "
...
if __name__ == "__main__":
    try:
        ...
        log.fatal(
            "Called program '%s' returned with %d.\n"
            "STDOUT:\n%s\n"
